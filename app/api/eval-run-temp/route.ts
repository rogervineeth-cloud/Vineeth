...
//
// Locked down:
//   - 404 after EXPIRES_AT, unconditionally — any deployment that outlives
//     the eval (deployments keep the env they were built with) goes inert
...
export async function GET(req: NextRequest) {
  if (Date.now() > Date.parse(EXPIRES_AT)) return notFound();
