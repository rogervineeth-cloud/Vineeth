  ...
  return (
    <div className="min-h-screen bg-[#f7f3ea]">
      ...
      <section className="max-w-6xl mx-auto px-6 pt-10 sm:pt-14 pb-16">
        ...
        {/* Feature strip */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-12 pt-10 border-t border-stone-200/60">
          <div className="flex items-start gap-4 p-4 rounded-xl transition-all duration-300 hover:bg-white hover:shadow-xl hover:-translate-y-1 group">
            <div className="w-10 h-10 rounded-lg bg-[#1f5c3a]/10 flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:bg-[#1f5c3a]/20 transition-all">
              <Zap className="w-5 h-5 text-[#1f5c3a]" />
            </div>
            <div>
              <p className="font-semibold text-sm text-[#1a1a1a]">Bullets written for you</p>
              <p className="text-xs text-[#6b6b6b] mt-1 leading-relaxed">Tailored to the job description, not generic templates.</p>
            </div>
          </div>
          <div className="flex items-start gap-4 p-4 rounded-xl transition-all duration-300 hover:bg-white hover:shadow-xl hover:-translate-y-1 group">
            <div className="w-10 h-10 rounded-lg bg-[#1f5c3a]/10 flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:bg-[#1f5c3a]/20 transition-all">
              <Target className="w-5 h-5 text-[#1f5c3a]" />
            </div>
            <div>
              <p className="font-semibold text-sm text-[#1a1a1a]">Matched to the JD</p>
              <p className="text-xs text-[#6b6b6b] mt-1 leading-relaxed">Keywords, skills, and tone aligned to what the recruiter wants.</p>
            </div>
          </div>
          <div className="flex items-start gap-4 p-4 rounded-xl transition-all duration-300 hover:bg-white hover:shadow-xl hover:-translate-y-1 group">
            <div className="w-10 h-10 rounded-lg bg-[#1f5c3a]/10 flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:bg-[#1f5c3a]/20 transition-all">
            ...
            <div>
              ...
              <p className="text-xs text-[#6b6b6b] mt-1 leading-relaxed">Single-column, clean formatting designed to parse cleanly.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Pricing */}
      <section id="pricing" className="max-w-5xl mx-auto px-6 py-16 border-t border-stone-200/60">
        ...
        {/* Free Banner */}
        <div className="max-w-2xl mx-auto mb-12 bg-white rounded-2xl border border-stone-200 p-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm hover:shadow-md transition-shadow">
          <div>
            <h3 className="font-semibold text-[#1a1a1a]">Not sure yet?</h3>
            <p className="text-sm text-[#6b6b6b] mt-1">Generate a watermarked preview and see your ATS score for free.</p>
          </div>
          <Button asChild variant="outline" className="shrink-0 border-[#1f5c3a] text-[#1f5c3a] hover:bg-[#1f5c3a]/5 transition-all hover:shadow-md hover:-translate-y-0.5">
            <Link href="/signup">Try Free Preview</Link>
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`relative rounded-2xl border p-6 flex flex-col gap-5 transition-all duration-300 ${plan.popular ? "border-[#1f5c3a] bg-[#1f5c3a] text-white shadow-xl hover:shadow-2xl hover:-translate-y-2 scale-100 sm:scale-105 z-10" : "border-stone-200 bg-white hover:shadow-lg hover:-translate-y-1 mt-4 sm:mt-0"}`}
            >
              {plan.popular && <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-400 text-black text-xs font-bold px-4 py-1 rounded-full whitespace-nowrap shadow-sm">Best value</span>}
              <div>
              ...
              <ul className="flex flex-col gap-3 text-sm flex-1 mt-2">
                {plan.features.map((f) => (
              ...
              <Button asChild variant={plan.popular ? "secondary" : "outline"} size="lg" className={`w-full mt-2 transition-all ${plan.popular ? "bg-white text-[#1f5c3a] hover:bg-white/90 hover:scale-105 shadow-md" : "hover:border-[#1f5c3a] hover:text-[#1f5c3a]"}`}>
                <Link href={`/signup?plan=${plan.name.toLowerCase().replace(/ /g, "_")}`}>Get started</Link>
              </Button>
            </div>
          ))}
        </div>
        <p className="text-center text-[#6b6b6b] text-sm mt-10">+ LinkedIn Profile Rewrite add-on available for ₹500</p>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-6 py-14 border-t border-stone-200/60">
        ...
        <div className="flex flex-col gap-3">
          {faqs.map((faq) => (
            <details key={faq.q} className="group border border-stone-200 rounded-xl bg-white overflow-hidden transition-all duration-300 hover:shadow-md">
              <summary className="px-6 py-5 cursor-pointer font-medium text-[#1a1a1a] list-none flex items-center justify-between gap-4 hover:bg-stone-50 transition-colors [&::-webkit-details-marker]:hidden">
                {faq.q}
                <span className="text-[#1f5c3a] shrink-0 group-open:rotate-45 transition-transform duration-300 text-2xl leading-none">+</span>
              </summary>
              <div className="px-6 pb-5 text-sm text-[#6b6b6b] leading-relaxed border-t border-stone-100">{faq.a}</div>
            </details>
          ))}
        </div>
      </section>
      ...
      {/* Footer */}
      <footer className="relative mt-12 py-10 px-6 text-center text-sm text-[#6b6b6b]">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-4xl h-px bg-gradient-to-r from-transparent via-stone-300 to-transparent" />
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-6">
          <span className="font-medium text-[#1a1a1a]">© 2026 Neduresume</span>
          <span className="hidden sm:inline text-stone-300">|</span>
          <span>Made with ❤️ in India</span>
          <span className="hidden sm:inline text-stone-300">|</span>
          <a href="mailto:rogervineeth@gmail.com" className="hover:text-[#1f5c3a] transition-colors font-medium">Contact Support</a>
        </div>
      </footer>
    </div>
  );
